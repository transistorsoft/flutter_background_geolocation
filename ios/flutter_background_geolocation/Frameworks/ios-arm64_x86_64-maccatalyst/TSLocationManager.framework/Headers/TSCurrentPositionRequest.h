...
@property (atomic, assign) TSLocationType type;

/// Maximum acceptable age (in seconds) for a cached location. 0 = must be fresh.
@property (atomic, assign) NSInteger maximumAge;
...
@property (atomic, assign) CLLocationAccuracy desiredAccuracy;

/// Number of samples to consider before deciding success (>=1).
@property (atomic, assign) NSInteger samples;

/// Allow returning a stale cached fix (subject to maximumAge).
@property (atomic, assign) BOOL allowStale;

/// Whether to persist the resulting TSLocationEvent to storage.
@property (atomic, assign) BOOL persist;
...
@property (atomic, copy, nullable) NSDictionary<NSString *, id> *extras;

/// Success callback (non-null event).
@property (atomic, copy, nullable) void (^success)(TSLocationEvent * _Nonnull event);

/// Failure callback (non-null error).
@property (atomic, copy, nullable) void (^failure)(NSError * _Nonnull error);
