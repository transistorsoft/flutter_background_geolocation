...
* [Fixed][Android] `ready()` on a later launch no longer switches a running scheduler off. Resetting the configuration briefly applied the empty default `schedule`, which stopped the scheduler and could start tracking outside the schedule window until your app called `startSchedule()` again. The scheduler now stays on.
...
* [Added] `ProviderChangeEvent.AUTHORIZATION_STATUS_DENIED_ALWAYS` (5) — the motion permission was permanently denied; only the device's Settings app can restore it (Android: after two user denials; iOS: a single motion denial is already permanent). Reported by `requestPermission(Permission.motion)` only — it never appears in `providerchange` events. (WO-007/WO-014)
* [Changed][iOS] The no-argument `requestPermission()` now also requests the motion permission after the location flow resolves (matching Android, where it always did) — the motion prompt moves from `start()` to `requestPermission()` for apps that pre-request. Apps that call `requestPermission()` but never `start()` will see a Motion & Fitness prompt they previously did not; ensure `NSMotionUsageDescription` is present in `Info.plist` (it has always been required for `start()`). Set `disableMotionActivityUpdates: true` to opt out of motion entirely. (WO-007)
* [Changed] ⚠️ Breaking: a denied `requestPermission()` now errors the `Future` with the bare authorization status `int` (previously a `PlatformException` with the status buried in `details`), matching the documented cross-platform contract. Typed `on PlatformException` handlers around `requestPermission()` will no longer match a denial — use an untyped `catch (status)` (as all documented examples always have) or `on int`. (WO-007)
* [Note] The new `Permission` enum shares its name with the popular `permission_handler` package. Apps importing both **unprefixed** will hit `ambiguous_import` — import this package with a prefix (the documented convention: `import '.../flutter_background_geolocation.dart' as bg;`) or use `show`/`hide`.
